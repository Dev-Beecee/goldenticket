...
const permit = new Permit({
  ...
  // your api key
  token:
  'permit_key_6zaqFaA2FqZuzcKODHADAkWyVnoEuIAdbIijZQUWOA7XiR8PeM0Krvn46iJMlOHw7tmelz92SpEaMvOffeNEDD',
});
...
const permitted = await permit.check('John@Doe.com', 'read', 'inscription');
  if (permitted) {
    res.status(200).send(`${user.firstName} ${user.lastName} is PERMITTED to 'read' 'inscription' !`);
  } else {
    res.status(403).send(`${user.firstName} ${user.lastName} is NOT PERMITTED to 'read' 'inscription' !`);
  }
});
